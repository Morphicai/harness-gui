...
/// Native SDK 框架的默认位置。
///
/// 原来这里写死过某台机器上的绝对路径 —— 换机器就构建不了，而且把用户名带进了仓库。
/// 现在按 npm 全局安装的常规位置探，都不命中就让 -Dnative-sdk-path 报明确错误。
const default_native_sdk_rel = "lib/node_modules/@native-sdk/cli";
const app_exe_name = "native-client";
...
pub fn build(b: *std.Build) void {
    ...
    const native_sdk_path = b.option([]const u8, "native-sdk-path", "Path to the Native SDK framework checkout") orelse resolveNativeSdkPath(b);
...
/// 找 Native SDK：先看 NATIVE_SDK_PATH，再按 npm 全局前缀的常规位置探。
///
